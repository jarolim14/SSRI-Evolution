...
class TextProcessor:
    ...
    def clean_text_and_remove_start_and_ending_statements(
        self, return_cleaned_text_separately=False
    ):
        """
        ...
        print("cleaned text and removed start and ending statements")

        self.df["title_abstract"] = [self.clean_text(t) for t in title_abstract]
        print("cleaned text - embed me now :)")
        if return_cleaned_text_separately:
            self.df["clean_title"] = [self.clean_text(t) for t in clean_title]
            self.df["clean_abstract"] = [self.clean_text(t) for t in clean_abstract]
            return self.df
        return self.df
