...
def smooth_segment(segments, tension, idx, idy, idz):
	...
	for i in range(1, seg_length):
		...
		current[idz] = ((1 - tension) * current[idz]) + (tension * (previous[idz] + next_point[idz]) / 2)
...
def advect_and_resample(vert, horiz, depth, segments, iterations, accuracy, min_segment_length, max_segment_length, segment_class):
	for it in range(iterations):
		advect_segments(segments, vert, horiz, depth, accuracy, segment_class.idx, segment_class.idy, segment_class.idz)
		if it % 2 == 0:
...
def advect_resample_all(gradients, edge_segments, iterations, accuracy, min_segment_length, max_segment_length, segment_class):
	...
	return [advect_and_resample(vert, horiz, depth, edges, iterations, accuracy, min_segment_length, max_segment_length, segment_class)
...
class UnweightedSegment(BaseSegment):
	...
	def create_segment(edge):
		return np.array([[edge[0], edge[1], edge[2], edge[3]], [edge[0], edge[4], edge[5], edge[6]]])

	@staticmethod
	@ngjit
	def accumulate(img, point, accuracy):
		img[int(point[1] * accuracy), int(point[2] * accuracy), int(point[3] * accuracy)] += 1
...
class EdgelessUnweightedSegment(BaseSegment):
	...
	@ngjit
	def accumulate(img, point, accuracy):
		img[int(point[0] * accuracy), int(point[1] * accuracy), int(point[2] * accuracy)] += 1
...
class WeightedSegment(BaseSegment):
	...
	@staticmethod
	def get_merged_columns(params):
		return ['edge_id', 'src_x', 'src_y', 'src_z', 'dst_x', 'dst_y', 'dst_z', params.weight]

	@staticmethod
	@nb.jit
	def create_segment(edge):
		return np.array([[edge[0], edge[1], edge[2], edge[3], edge[7]], [edge[0], edge[4], edge[5], edge[6], edge[7]]])

	@staticmethod
	@ngjit
	def accumulate(img, point, accuracy):
		img[int(point[1] * accuracy), int(point[2] * accuracy), int(point[3] * accuracy)] += point[4]
...
class EdgelessWeightedSegment(BaseSegment):
	...
	@nb.jit
	def create_segment(edge):
		return np.array([[edge[0], edge[1], edge[2], edge[6]], [edge[3], edge[4], edge[5], edge[6]]])

	@staticmethod
	@ngjit
	def accumulate(img, point, accuracy):
		img[int(point[0] * accuracy), int(point[1] * accuracy), int(point[2] * accuracy)] += point[3]
	...
	def __call__(self, nodes, edges, **params):
		...
		for i in range(20):
			for batch in edge_segments:
				smooth(batch, p.tension, segment_class.idx, segment_class.idy, segment_class.idz)
...
if __name__ == '__main__':
	...
	cocitenodes = [[nid, nposdict['x'], nposdict['y'], float(nposdict['size'])*10 ] for nid, nposdict in nodepos.items()]
	...
	cocite_bundled = hammer_bundle(cocitenodes, cociteedges, initial_bandwidth=0.60, decay = 0.7, accuracy=5*10**2,
							  weight = None, batch_size = 300000,
							  advect_iterations=50, iterations=10,
							  min_segment_length=0.005, max_segment_length=0.05)
	...
	with open('cocite3Dedges_bw06.csv', 'w') as ofile:
		for irow in range(npts - 1):
			...
			if np.isnan(x).sum() > 0:
				filestr = ', '.join(map(str, cociteedges[['source', 'target']].iloc[iedge].values)) + ', '
